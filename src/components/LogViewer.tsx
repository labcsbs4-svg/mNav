...
export const LogViewer: React.FC = () => {
  ...
  return (
    <div className={`fixed top-4 right-4 bg-white rounded-lg shadow-lg transition-all duration-300 z-[2000] ${
      isExpanded ? 'w-96 h-96' : 'w-40 h-10'
    }`}>
      {/* Header */}
      <div className={`flex items-center justify-between p-2 border-b ${isExpanded ? 'bg-blue-500' : 'bg-blue-600'} rounded-t-lg`}>
        ...
        <div className="overflow-auto h-[calc(100%-2.5rem)] p-2">
          <div className="space-y-2">
            {filteredLogs.length === 0 ? (
              <p className="text-gray-500 text-center py-4">No logs to display</p>
            ) : (
              filteredLogs.map((log, index) => (
                <div key={index} className="text-sm border rounded p-3 hover:bg-gray-50 transition-colors">
