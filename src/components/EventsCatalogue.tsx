...
const Logger = ({ logs, onClear }: { logs: string[]; onClear: () => void }) => {
  ...
  return (
    <div className="hidden fixed bottom-4 right-4 bg-black bg-opacity-80 text-white p-4 rounded-lg max-w-md max-h-64 overflow-y-auto z-50">
...
export default function EventsCatalogue({
  ...
  const filteredEvents = useMemo(() => {
    ...
    addLog(
      `Computing filteredEvents: ${events.length} total -> ${filtered.length} filtered (search: "${searchQuery}", category: "${selectedCategory}")`
    );
    return filtered;
                        ...
                        <div>
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              setEventFormData({
                                ...eventFormData,
                                registeredCount: Number(e.target.value),
                              })
                            }
                            className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                          />
            ...
            {filteredEvents.length === 0 ? (
              <div className="text-center py-12">
                ...
                <p className="text-sm text-gray-400 mt-2">
                  Debug: Total events in state: {events.length}
                </p>
              </div>
            ) : (
                      ...
                      <div className="flex justify-between items-start mb-3">
                        <span
                          className={`inline-block px-3 py-1 rounded-full text-xs font-medium border ${
                            (categoryColors as any)[event.category]
                          }`}
                        >
                          ...
                          <span
                            className={`font-medium ${
                              getRegistrationStatus(event) === "Full"
                                ? "text-red-600"
                                : getRegistrationStatus(event)?.includes(
                                    "spots left"
                                  )
                                ? "text-orange-600"
                                : "text-blue-600"
