    ...
    <div className="bg-white rounded-xl shadow-lg p-6">
      ...
      <div className="space-y-3">
            ...
            {instructions.length === 0 ? (
              ...
              instructions.map((ins, idx) => (
                <div key={idx} className="flex items-start space-x-2">
                  <span className="w-6 h-6 bg-blue-100 text-blue-800 rounded-full flex items-center justify-center text-xs font-medium mt-0.5">{idx + 1}</span>
                  <p>{ins}</p>
                </div>
              ))
            )}
        ...
        <div className="pt-3 border-t">
          ...
          <p className="text-xs text-gray-500 text-center mt-2">Press Esc to stop</p>
        </div>
      </div>
    </div>
  );
