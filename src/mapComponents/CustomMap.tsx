...
import { Plus, Minus, RotateCcw, RotateCw, Maximize, Minimize } from 'lucide-react';
  ...
  return L.divIcon({
    html: `
      <div style="display: flex; flex-direction: column; align-items: center; transform: translate(-50%, -100%);">
        <div style="background: white; border-radius: 50%; width: 18px; height: 18px; display: flex; align-items: center; justify-content: center; border: 2px solid ${categoryColors[category as keyof typeof categoryColors]}; box-shadow: 0 2px 6px rgba(0,0,0,0.15); font-size: 9px;">
...
const MapEventsHandler: React.FC<MapEventsHandlerProps> = ({ mode, onMapClick, onContextMenu }) => {
  useMapEvents({
    click: (e) => {
...
export const CustomMap: React.FC = () => {
  ...
  const [selectedFromPoint, setSelectedFromPoint] = useState<Location | null>(null);
  ...
  const [hasCenteredOnLiveLocation, setHasCenteredOnLiveLocation] = useState<boolean>(false);
  const { location: userLocation, loading: userLocationLoading, error: userLocationError } = useGeolocation({}, liveLocationEnabled);
  ...
  React.useEffect(() => {
    ...
    return () => window.removeEventListener('toggle-geo-menu', handler as EventListener);
  }, []);
  ...
  React.useEffect(() => {
    ...
    return () => window.removeEventListener('toggle-live-location', handler as EventListener);
  }, []);
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('request-one-off-location', handler as EventListener);
    return () => window.removeEventListener('request-one-off-location', handler as EventListener);
  }, []);

  // When live location is first enabled, center the map on user location
  React.useEffect(() => {
    if (liveLocationEnabled && userLocation && mapRef.current && !hasCenteredOnLiveLocation) {
  ...
  React.useEffect(() => {
    ...
    return () => window.removeEventListener('navigation-stopped', handleNavigationStopped);
  }, []);

  // Notify outside world when live location toggles so parent UI can reflect state
  React.useEffect(() => {
    window.dispatchEvent(new CustomEvent('live-location-changed', { detail: { enabled: liveLocationEnabled } }));
  }, [liveLocationEnabled]);
  ...
  React.useEffect(() => {
    ...
    const dest = mapState.currentRoute.waypoints[mapState.currentRoute.waypoints.length - 1];
    const remaining = haversineDistance(userLocation[0], userLocation[1], dest[0], dest[1]);
    ...
    if (mapState.currentRoute && navigationInstructions.length > 1) {
      const nextWaypointIndex = mapState.currentRoute.waypoints.length - (navigationInstructions.length - 1);
      if (nextWaypointIndex < mapState.currentRoute.waypoints.length) {
        ...
        const distanceToNextWaypoint = haversineDistance(userLocation[0], userLocation[1], nextWaypoint[0], nextWaypoint[1]);
  ...
  }, [userLocation, mapState.isNavigating, mapState.currentRoute, navigationInstructions]);
    ...
    (async () => {
      try {
        ...
        if (locRes.ok) {
          ...
          setLocations(locs.map((l: any) => ({ ...l, createdAt: new Date(l.createdAt) })));
        }
        if (roadRes.ok) {
          const rds = await roadRes.json();
          setRoads(rds.map((r: any) => ({ ...r, createdAt: new Date(r.createdAt) })));
        }
      } catch (err) {
  ...
  const retryBackend = async () => {
    try {
      ...
      if (res.ok) {
        ...
        const [locRes, roadRes] = await Promise.all([fetch('/api/locations'), fetch('/api/roads')]);
        if (locRes.ok) setLocations((await locRes.json()).map((l: any) => ({ ...l, createdAt: new Date(l.createdAt) })));
        if (roadRes.ok) setRoads((await roadRes.json()).map((r: any) => ({ ...r, createdAt: new Date(r.createdAt) })));
      }
    } catch (e) {
      setBackendOnline(false);
    }
  ...
  const setManualLocation = () => {
    const input = window.prompt('Enter coordinates as "lat,lng" (for example: 40.7128,-74.0060)');
    if (!input) return;
  ...
  fetch(`/api/locations/${id}`, { method: 'DELETE' })
      .then(res => {
        if (!res.ok) logger.error('Delete location failed', { status: res.status });
      })
  ...
  const handleSaveRoad = useCallback((name: string) => {
    ...
    (async () => {
      try {
        const res = await fetch('/api/roads', {
          ...
          body: JSON.stringify({ name, type: 'custom', coordinates: tempRoadPoints }),
        });
        if (res.ok) {
          const created = await res.json();
          const parsed = { ...created, createdAt: new Date(created.createdAt) };
  ...
  const handleMapClick = useCallback((lat: number, lng: number) => {
    ...
    switch (mode) {
      ...
      case 'delete-location':
        ...
        logger.info('Opening deletion modal for locations', { preselected: nearestLocId });
        break;
      case 'delete-road':
        ...
        setIsDeletionModalOpen(true);
        logger.info('Opening deletion modal for roads', { preselected: nearestRoadId });
        break;
    }
  ...
  const findNearestLocationId = useCallback((lat: number, lng: number) => {
  ...
  const findNearestRoadId = useCallback((lat: number, lng: number) => {
  ...
  const res = await fetch('/api/locations', {
        ...
        if (res.ok) {
          ...
          const body = await res.text().catch(() => '<unreadable>');
          logger.error('Failed to save location', { status: res.status, body });
          console.error('Failed to save location', { status: res.status, body });
      ...
      (async () => {
        try {
          const res = await fetch('/api/roads', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ name: `Road ${roads.length + 1}`, type: 'custom', coordinates: currentRoadPoints }),
          });
          if (res.ok) {
            const created = await res.json();
            const parsed = { ...created, createdAt: new Date(created.createdAt) };
  ...
  const handleStopNavigation = useCallback(() => {
    ...
    setMapState(prev => ({ ...prev, isNavigating: false, currentRoute: null }));
  ...
  const isRoadPartOfRoute = useCallback((road: Road, routeWaypoints: [number, number][]) => {
    ...
    for (let i = 0; i < routeWaypoints.length - 1; i++) {
      ...
      for (const [roadLat, roadLng] of road.coordinates) {
        ...
        const distToStart = haversineDistance(roadLat, roadLng, startLat, startLng);
        const distToEnd = haversineDistance(roadLat, roadLng, endLat, endLng);
        const segmentLength = haversineDistance(startLat, startLng, endLat, endLng);
  ...
  const computeRouteAndInstructions = useCallback((start: [number, number], dest: [number, number]) => {
    ...
    if (roadResult && roadResult.waypoints.length > 1) {
      ...
      // Initial instruction
      const initialBearing = calculateBearing(waypoints[0][0], waypoints[0][1], waypoints[1][0], waypoints[1][1]);
      const initialDirection = bearingToDirection(initialBearing);
      const firstSegmentDistance = haversineDistance(waypoints[0][0], waypoints[0][1], waypoints[1][0], waypoints[1][1]);
      instructions.push(`Head ${initialDirection} for ${Math.round(firstSegmentDistance)} m.`);
      ...
      for (let i = 1; i < waypoints.length - 1; i++) {
        ...
        const bearingIn = calculateBearing(prevPoint[0], prevPoint[1], currentPoint[0], currentPoint[1]);
        const bearingOut = calculateBearing(currentPoint[0], currentPoint[1], nextPoint[0], nextPoint[1]);
        ...
        const segmentDistance = haversineDistance(currentPoint[0], currentPoint[1], nextPoint[0], nextPoint[1]);
        if (turnDirection) {
           instructions.push(`${turnDirection}, then continue for ${Math.round(segmentDistance)} m.`);
        }
      }
    ...
    const instructions = [`Head towards destination (${distance < 1000 ? `${Math.round(distance)} m` : `${(distance/1000).toFixed(1)} km`})`];
    const newRoute: Route = {
      id: Date.now().toString(),
  ...
  const handleSelectDestination = useCallback(async (loc: Location) => {
    if (navigationStep === 'from') {
      ...
      if (selectedFromPoint?.id === 'user-location') {
        ...
        start = userLocation || [40.7128, -74.0060]; // Fallback to default
        if (!userLocation) {
          if (!navigator.geolocation) {
            alert('Geolocation not supported');
            return;
      ...
      const { route: computedRoute, instructions, distance } = computeRouteAndInstructions(start, [loc.lat, loc.lng]);
  ...
  }, [navigationStep, selectedFromPoint, userLocation, computeRouteAndInstructions]);
  ...
  const handleConfirmStartNavigation = useCallback(() => {
    ...
    if (routes.length > 0) {
      ...
      setMapState(prev => ({ ...prev, isNavigating: true, currentRoute: latestRoute }));
      // Dispatch navigation start event
      window.dispatchEvent(new CustomEvent('navigation-started', { detail: { route: latestRoute } }));
  ...
  }, [mode, currentRoadPoints, routeWaypoints, handleFinishRoad, handleFinishRoute, mapState.isNavigating]);
  ...
  return (
    <div ref={mapContainerRef} className={`relative w-full h-full overflow-hidden ${getCursorStyle()}`}>
      {mapState.isNavigating && navigationInstructions.length > 0 && (
        <div className="absolute top-4 left-1/2 transform -translate-x-1/2 z-[1000] bg-white rounded-lg shadow-lg p-4 max-w-sm w-full">
          <p className="text-lg font-semibold text-gray-900 text-center">{navigationInstructions[0]}</p>
          {navigationInstructions.length > 1 && (
            <p className="text-sm text-gray-600 text-center mt-1">Next: {navigationInstructions[1]}</p>
          )}
        </div>
      )}
      <div 
        ...
        {showRoads && (
          <React.Fragment key={roadsKey}>
            {roads.map((road) => {
              ...
              const isCurrentRoad = mapState.isNavigating && mapState.currentRoute && 
      ...
      <ConfirmNavigationModal
        ...
        onCancel={() => { setShowConfirmNavModal(false); }}
      />
      ...
      {isGeoMenuOpen && (
        <div className="fixed top-24 right-6 z-[1000] bg-white rounded-md shadow p-3 text-sm max-w-xs">
          <div className="flex items-start justify-between mb-2">
            ...
            <button aria-label="Close location menu" onClick={() => setIsGeoMenuOpen(false)} className="text-gray-400 hover:text-gray-600 text-xs">✕</button>
          </div>
          <div className="mb-2">
            {userLocationLoading ? (
          ...
          <div className="flex space-x-2">
            <button onClick={requestOneOffLocation} className="px-2 py-1 bg-blue-600 text-white rounded text-xs">Request location</button>
            <button onClick={setManualLocation} className="px-2 py-1 bg-yellow-100 text-yellow-800 rounded text-xs">Set manually</button>
            
            <button
              onClick={() => window.open('https://developer.mozilla.org/en-US/docs/Web/API/Geolocation_API#permissions_and_privacy', '_blank')}
