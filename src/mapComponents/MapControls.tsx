...
import { Route, Navigation, Plus, Eye, EyeOff, Trash2, Compass } from 'lucide-react';
  ...
  return (
    <div className="absolute top-4 left-4 z-[1000] bg-white rounded-lg shadow-lg p-2 space-y-2">
      <div className="flex flex-col space-y-1">
        ...
        <button
          onClick={() => setMode(mode === 'delete-location' ? 'view' : 'delete-location')}
          className={`flex items-center space-x-2 px-3 py-2 rounded-md text-sm font-medium transition-colors ${
        ...
        <button
          onClick={() => setMode(mode === 'delete-road' ? 'view' : 'delete-road')}
          className={`flex items-center space-x-2 px-3 py-2 rounded-md text-sm font-medium transition-colors ${
            mode === 'delete-road'
              ? 'bg-red-600 text-white'
              : 'text-gray-700 hover:bg-gray-100'
